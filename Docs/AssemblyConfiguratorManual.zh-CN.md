...
- 不显示方向箭头。
- 手靠近高亮零件时，绿色明确切换为蓝色交互反馈；移开后恢复绿色。
- 当前 round 全部交互完成后，高亮推进到下一 round。

### CurrentPartHighlightAndDirection

- 包含 `CurrentPartHighlight` 的全部行为。
- 当前 round 的每个未完成零件额外显示较小的半透明琥珀色 Shapes 三维箭头。
- 箭头方向来自 txt，并且相对于模型索引父物体。

注意：三种条件下，所有 txt 零件始终都可以抓取。TXT round 只决定绿色高亮和琥珀色箭头，不会锁住未来零件。
...
- `CurrentPartHighlightAndDirection`：当前 round 同时有绿色高亮和半透明琥珀色箭头。
...
这是正确的视觉优先级：蓝色是当前真实交互反馈，绿色是拆卸顺序指引。移开手后应恢复绿色。如果始终保持绿色，请确认重新一键配置并等待脚本编译完成；现在状态切换会显式刷新 Highlight Plus 材质。
